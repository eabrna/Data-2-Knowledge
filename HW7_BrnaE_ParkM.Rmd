...
matplot(0:1000, cum.phat, type="l", xlab="Number of tosses (n)", ylab="Estimated Proportion of heads", col = rainbow(20))
abline(h=0.5)

n_seq <- 0:1000
Ephat <- rep(0.5, length(n_seq)) # E(Y) is 0.5 for all n
SDphat <- 1 / (2 * sqrt(n_seq)) 

lines(n_seq, Ephat, col = "black",lwd = 5)          
lines(n_seq, Ephat+2*SDphat, lwd=2)   
lines(n_seq, Ephat-2*SDphat, lwd=2) 
```
...
```{r}
p = 0.5
cum.win <- matrix(NA, nrow=1000, ncol=20)
for (i in 1:20) {
  X <- cumsum(rbinom(1000, 1, 0.5))
  pwin <- X / (1:1000)
  cum.win[, i] <- pwin
...
cum.win <- rbind(0, cum.win)
matplot(0:1000, cum.win, type="l", xlab="Number of games", ylab="Estimated Probability of Winning", col = rainbow(20))
abline(h=0.5)

n_seq <- 0:1000
Ephat <- rep(p, length(n_seq))
SDphat <- 2*sqrt(p*(1-p)/n_seq)
