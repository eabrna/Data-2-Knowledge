...
```
The p-value is smaller than the significance level (0.01099363 < 0.05) therefore we reject the null hypothesis. There is significant evidence that the mean population score is differs from 500.
...
alpha = prop.test(7, n, p=.5, alternative = "less")$p.value*2;
cat("Probability of Type 1 error (normal distribution):", alpha, "\n")

# using a binomial distribution
alpha.binomial = binom.test(7, 25, alternative = "less")$p.value + binom.test(18, 25, alternative = "greater")$p.value
...
p = c(.3, .4, .6, .7)
beta = pnorm(.5, mean = p, sd = (sqrt(p*(1-p)/n)))
power = 1-beta; power

# (iv)
  # x = 6 is outside the first rejection region allowing you to reject the null hypothesis and concluded that one company is favored more than the other. 
```

**should 'appreciate' be appropriate??
...
c. The pairings of one-two, one-three, two-three, two-four, and three-four differ significantly (difference_ij > w_ij).
