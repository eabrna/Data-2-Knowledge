...
vec = round(runif(50, min=0, max=10), 0) #generates random vector

table(vec)

table(vec) == max(table(vec))

which(table(vec) == max(table(vec)))
...
par(mfcol=c(1,2))
boxplot(twins$Foster, main="Boxplot of Foster Twins")
boxplot(twins$Biological,  main="Boxplot of Biological Twins")

```
The boxplots for both the foster and biological twins appears to have very similar spreads, with the center of the boxplot for foster twins being slightly higher than that of the biological plot.
